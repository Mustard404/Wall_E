...
func NewRabbitMQClient(queueName string) (*RabbitMQClient, error) {
	// 连接到RabbitMQ服务器
	conn, err := amqp.Dial("amqp://" + AmpqConfig.User + ":" + AmpqConfig.Password + "@" + AmpqConfig.Host +
		":" + AmpqConfig.Port + "/" + AmpqConfig.Vhost)
	if err != nil {
		return nil, err
	}
...
func (c *RabbitMQClient) Consume() {
	...
	go func() {
		for d := range msgs {
			var portInfo Port
			err := json.Unmarshal(d.Body, &portInfo)
			if err != nil {
...
// MQMessageCount 返回指定队列的消息数量
func MQMessageCount(queueName string) (int, error) {
	url := fmt.Sprintf("http://%s:%s@%s:%s/api/queues/%s/%s",
		AmpqConfig.User, AmpqConfig.Password, AmpqConfig.Host, AmpqConfig.ApiPort, AmpqConfig.Vhost, queueName)
	...
	// 提取消息总数
	messages, ok := queueInfo["messages"].(float64)
	if !ok {
		return -1, fmt.Errorf("error retrieving messages count")
	}
